 ...
 */
class AdminsLoginForm extends Model
{
    public $username;
    public $email;
    public $password;
    ...
    public function getUser()
    {
        if ($this->_user === false) 
        {
            ...
            {
                $this->_user = Admins::findByEmail($this->email);
            } 
            else 
            {
    ...
    public function notActivated()
    {
        ...
        if ($user = Admins::userExists($username, $this->password, $this->scenario))
        {
            if ($user->status === Admins::STATUS_NOT_ACTIVE)
            {
                return true;
            }
            else
